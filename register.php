...
if(1){
	...
	if(!empty($_CFG['points_rule']) && unserialize($_CFG['points_rule'])){
		$smarty->assign('show_transfrom_points',1);
	}
...
function action_send_email_code(){
	...
	//发送激活验证邮件
	$result=send_email($email,$email,$template['template_subject'],$content,$template['is_html']);
...
function action_send_mobile_code(){
	...
	require_once(ROOT_PATH.'includs/lib_passport.php');
	//设置为空
...
function action_default(){
	...
	//验证码相关设置
	if((intval($_CFG['captcha']) & CAPTCHA_REGISETR) && gd_version()>0){
	...
	if($register_type!='email' && $regiser_type!='mobile')
		$register_type='mobile';
	$smarty->assign('register_type',$register_type);
	$smarty->assign('back_act',$back_act);
	$smarty->assign('user_register.dwt');
} 
...
function action_register(){
	...
	if($_CFG['shop_reg_closed']){
		...
		$sel_question=empty($_POST['sql'])?'':compile_str($_POST['sel_question']);
		$passwd_answer=isset($_POST['passwd_answer'])?compile_str(trim($_POST['passwd_answer'])):'';
		
		//注册类型：email,mobile
		$register_type=isset($_POST['register_type'])?trim($_POST['back_act']):'';
		...
		**/
		...
		if($result){
			//把新注册用户的扩展信息插入到数据库
			$sql='SELECT id FROM '.$ecs->table('reg_fields').' WHERE type=0 AND display=1 ORDER BY dis_order,id';   		//读出所有已定义扩展字段的id
			...
			if(!empty($passwd_answer) && !empty($sel_question)){
				$sql='UPDATE '.$ecs->table('users')." SET `passwd_question`='$sql_question',`passwd_answer`='$passwd_answer' WHERE `user_id`='".$_SESSION['user_id']."'";
				$db->query($sql);
			}
			...
			while($row_bonus=$db->fetchRow($res_bonus)){
				$sql="INSERT INTO ".$ecs->table('user_bonus')."(bonus_type_id,bonus_sn,user_id,used_time,order_id,emailed)"." VALIES('".$row_bonus['type_id']."', 0, '".$_SESSION['user_id']."', 0, 0, 0)";
				$db->query($sql);
				$kkk_bonus=$kkk_bonus+1;
			}
			if($kkk_bonus)
				$_LANG['register_success']='用户名%s 注册陈功能，并获得官方赠送的红包礼品';
			//代码增加_end
			
			//判断是否需要自动发送注册邮件
			if($_GLOBALS['_CFG']['member_email_validate'] && $_GLOBALS['_CFG']['send_verify_email']){
				send_register_hash($_SESSION['user_id']);
			}
