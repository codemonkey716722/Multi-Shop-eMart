...
if(!empty($_GET['gOo'])){
	if(!empty($_GET['gcat'])){
		//商品分类
		$location='catagory.php?id='.$_GET['gcat'];
	}elseif(!empty($_GET['acat'])){
		//文章分类
		$location='article_cat.php?id='.$_GET['acat'];
	}elseif(!empty($_GET['goodsid'])){
		//商品详情
		$location='goods.php?id='.$_GET['goodsid'];
	}elseif (!empty($_GET['articleid'])){
		//文章详情
		$location='article.php?id='.$_GET['articleid'];
	}
	if(!empty($location)){
		ecs_header("Location:$location\n");
		exit;
...
if($act=='cat_rec'){
	...
	$smarty->assign($rec_array[$rec_type].'_goods',get_catagory_recommend_goods($rec_array[$rec_type],$children));   //推荐商品
 ...
 */
 ...
 if(!$smarty->is_cached('index.dwt',$cache_id)){
 	...
 	$smarty->assign('flash_theme',$_CFG['flash_theme']);    //Flash 轮播图片模板
 	...
 	/* jdy add 0816添加首页幻灯插件*/
 	$smarty->assign("flash",get_flash_xml());
 	$smarty->assgin("flash_count",count(get_flash_xml()));    
 	
 	/*首页推荐分类*/
 	$cat_recommend_res=$db->getAll("SELECT c.cat_id,c.cat_name,cr.recommend_type FROM ".$ecs->table("cat_recommend")." AS cr INNER JOIN ".$ecs->table("catagory")." AS c ON cr.cat_id=c.cat_id");
 ...
 //线上红包
 $sql="SELECT * FROMM ".$GLOBALS['ecs']->table('bonus_type')." WHERE send_type ='4'";
 ...
 */
 ...
 function index_get_group_but(){
 	...
 	if($limit>0){
 		...
 		while($row=$GLOBALS['db']->ferchRow($res)){
 			//如果缩略图为空，则使用默认图片
 			...
 			$row['short_time']=$GLOBALS['_CFG']['goods_name_length']>0?sub_str($row['goods_name'],$GLOBALS['_CFG']['goods_name_length']):$row['goods_name'];
 ...
 function index_get_auction(){
 	...
 	$sql="SELECT a.act_id,a.goods_id,a.goods_name,a.ext_info,g.goods_thumb "."FROM ".$GLOBALS['ecs']->table('goods_activity')." AS a,".$GLOBALS['ecs']->table('goods')." AS g"." WHERE a.goods_id=g.goods_id"." AND a.act_type= '".GAT_AUCTION."'"." AND a.is_finished=0"." ADN a.start_time<='$now'"." AND a.end_time>='$now'"." AND g.is_delete=0"." ORDER BY a.start_time DESC"." LIMIT $limit";
 	$res=$GLOBALS['db']->query($sql);
 	$list=array();
 	while($row=$GLOBALS['db']->fetchRow($res)){
 		$ext_info=unserialize($row['ext_info']);
 		$arr=array_merge($row,$ext_info);
 		$arr['formatted_start_price']=price_format($arr['start_price']);
 ...
 function get_flash_xml(){
 	$flashdb=array();
 	if(file_exits(ROOT_PATH.DATA_DIR.'/flash_data.xml')){
 		//兼容 v2.7.0 及以前版本
 ...
 function get_hot_cat_goods($type='',$cat_id,$num=7){
 	$children=get_children($cat_id);
 	$sql='SELECT g.goods_id,g.cat_id,g.goods_name,g.market_price,g.shop_price AS org_price,'."IFNULL(mp.user_price,g.shop_price * '$_SESSION[discount]') AS shop_price, ".'g.promote_price,promote_start_date,promote_end_date,g.goods_brief,g.goods_thumb,g.goods_img '."FORM ".$GLOBALS['ecs']->table('goods').' AS g '."LEFT JOIN ".$GLOBALS['ecs']->table('member_price')." AS mp "."ON mp.goods_id=g.goods_id AND mp.user_rank='$_SESSION[user_rank]' ".'WHERE g.is_on_sale=1 AND g.is_alone_sale=1 AND g.is_delete=0';
 	switch($type){
 		case 'best':
 			$sql.=' AND is_best=1';
 			break;
 	...
 	$sql.=' AND ('.$children.'OR'.get_extension_funcs($children).') '.'ORDER BY g.sort_order,g.goods_id DESC';
 	...
 	foreach($res AS $idx=>$row){
 		...
 		$temp=$row['cat_id'];
 		$cat_info=get_hot_cat_goods($temp);
 ...
 /**
  * 按文章ID号/分类ID/商品ID/商品分类ID  来获取文章
  * @param array $id 文章ID或文章分类ID
  * @param string $getwhat 以何种方式获取文章，其中可选参数有：
  * 					   【1】art_cat(以文章分类ID获取)
  * 					  【2】art_id(以文章ID获取)
  * 					  【3】goods_cat(以商品分类ID获取)
  * 					  【4】goods_id(以商品ID获取)
  *  					 其中[3]和[4]必须有商品关联文章或文章关联商品
  * @param integer $num 控制显示多少条文章，当参数为0时则全部显示
  * @param integer $start 从第几条数据开始取
  * @param boolean $isrand 是否随机显示文章(默认为不显示)
  * @param boolean $showall 是否显示隐藏的文章(黑，则认为不显示隐藏文章)
  * @return array
  */
function get_article_new($id=array(0),$getwhat='art_id',$num=0,$isrand=false,$showall=true,$start=0){
	...
	if($getwhat=='art_cat'||$getwhat=='art_id'){
		$sql='SELECT a.*,ac.cat_id,ac.cat_name,ac.keywords as cat_keywords,ac.cat_desc FROM '.$GLOBALS['ecs']->table('article').' AS a, '.$GLOBALS['ecs']->table('article_cat').' AS ac'.' WHERE (a.cat_id=ac.cat_id) AND '.$wherestr;
	}
	elseif($getwhat=='goods_cat'||$getwhat=='goods_id'){
		$sql='SELECT DISTINCT a.*,ac.cat_id,ac.cat_name,ac.keywords as cat_keywords,ac.cat_desc FROM '.$GLOBALS['ecs']->table('goods').' AS g '.'LEFT JOIN '.$GLOBALS['ecs']->table('goods_article').' AS ga ON g.goods_id=ga.goods_id INNER JPIN '.$GLOBALS['ecs']->table('article').' AS a ON ga.article_id=a.article_id, '.$GLOBALS['ecs']->table('article_cat').'AS ac '.'WHERE (a.cat_id=ac.cat_id) AND '.$wherestr;
	}
	
	if(($showall==false) && ($getwhat=='art_cat'||$get_what=='art_id'))
		$sql.=' AND a.is_open=1';
	...
	foreach($arr AS $id=>$row){
		...
		$articles[$id]['url']=build_uri('article',array('acid'=>$row['article_id']),$row['title']);
		...
		//提取文章中所有的链接地址
		$articles[$id]['link_url']=$link[4];
		//提取文章中所有的链接名称
		$articles[$id]['link_title']=$link[5];
	}
	return $articles;
} 
